import { isCredibleCjPromotion, isOfferActive, normalizeCjOffer, type NormalizedOffer } from './offers.ts';
...
export async function syncCjOffers(env: OffersEnvironment, mode: CjSyncMode, now = new Date()): Promise<CjSyncResult> {
  ...
  for (let page = 1; calls < MAX_CALLS; page += 1) {
    ...
    linksFetched += parsed.records.length;
    try {
      fetched.push(...parsed.records
        .filter((record) => record.advertiserId && record.linkId && isCredibleCjPromotion(record))
        .map((record) => normalizeCjOffer(record, now)));
    } catch {
      throw new CjSyncError('parsing_normalization_failure', 'Unable to classify or normalize CJ offers');
    }
  ...
  if (!paginationComplete) throw new CjSyncError('pagination_safety_failure', 'CJ results exceeded the per-run page budget');
  ...
  console.log(JSON.stringify({ event: 'cj_offer_sync', mode, linksFetched, offersClassified: fetched.length, offersStored: offers.length, calls, syncedAt: snapshot.syncedAt }));
  return { offersFetched: linksFetched, offersStored: offers.length, pagesFetched: calls, mode, syncedAt: snapshot.syncedAt };
}
